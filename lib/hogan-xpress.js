...
var extend = function(dest, src) {
  ...
  while (i--) {
    var k = keys[i];
    if (dest[k] && src[k] && src[k].constructor === Object && dest[k].constructor === Object) {
      dest[k] = extend(dest[k], src[k]);
    } else {
      dest[k] = src[k];
    }
  }
...
var render = function(path, opt, fn) {
  ...
  var handleLambda = function(name, lambda) {
    ...
    opt.lambdas[name] = function() {
      ...
      return function(text) {
        ...
        var lctx = extend(JSON.parse(JSON.stringify(opt)), lambdaContext);
  ...
  renderPartials(partials, opt, function(err, partials) {
    ...
    var layout = typeof opt.layout === 'undefined' || opt.layout === null ? opt.settings.layout : opt.layout;
